...
export async function executeQuery(query, params = []) {
  try {
    ...
    console.error("Supabase query failed:", result?.error || "Unknown error");
    return { success: false, data: [], error: result?.error || "Query failed" };
  } catch (error) {
...
export async function fetchArticles(filters = {}) {
  ...
  if (filters.selected === true) {
    conditions.push(`selected_for_newsletter = true`);
  }
  
  if (filters.published === true) {
    conditions.push(`published_in_newsletter = true`);
  } else if (filters.published === false) {
    conditions.push(`published_in_newsletter = false`);
  }
  ...
  // Transform Supabase data to match our article model
